...
from acquisition_fn import mean_std, bald, max_entropy, variation_ratio, random_acquisition
...
def main_reproduction():
    ...
    for acquisition_fn_name, acquisition_fn in acquisition_fns.items():
        ...
        acc, std = avg_acc_std_AL(repetitions_al, acquisition_fn, train_set, pool_set, val_loader, test_loader, al_iterations,
        ...
        np.save(f"results/{acquisition_fn_name}_std.npy", std)


    acquisition_fns_det = {
        'bald_deterministic': bald,
        'var_ratio_deterministic': variation_ratio,
        'max_entropy_deterministic': max_entropy,
    }

    for acquisition_fn_name, acquisition_fn in acquisition_fns_det.items():
        print(f'Starting on determining accuracy for {acquisition_fn_name}')
        acc, std = avg_acc_std_AL(repetitions_al, acquisition_fn, train_set, pool_set, val_loader, test_loader, al_iterations,
                                  data_transfer_amount, seed, device, deterministic=True)
        print(f'Average accuracies and std for {acquisition_fn_name}:')
        print(f'{acc}')
        print(f'{std}')
        np.save(f"results/{acquisition_fn_name}_acc.npy", acc)
